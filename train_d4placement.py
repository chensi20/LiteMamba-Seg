import os

import torch
import torch.nn as nn
import torch.optim as optim
import torch.nn.functional as F
from tqdm import tqdm

try:
    from torch.amp import GradScaler
except ImportError:
    from torch.cuda.amp import GradScaler

from config import Config
from dataset import get_loaders
from models.D4Placement import D4Placement_UNet as UNet
from utils.losses import StructureLoss
from utils.metrics import calculate_metrics
from utils.tools import seed_everything


class BoundaryIoULoss(nn.Module):
    def __init__(self):
        super(BoundaryIoULoss, self).__init__()

    def forward(self, pred, target):
        """
        Sample-wise Boundary IoU Loss.

        pred:
            Raw logits, shape [B, 1, H, W]

        target:
            Binary masks, shape [B, 1, H, W]
        """

        # FP32 computation for numerical stability under AMP
        pred = torch.sigmoid(pred.float())
        target = target.float()

        if target.ndim == 3:
            target = target.unsqueeze(1)

        if pred.ndim == 3:
            pred = pred.unsqueeze(1)

        eroded = 1.0 - F.max_pool2d(
            1.0 - target,
            kernel_size=3,
            stride=1,
            padding=1,
        )

        dilated = F.max_pool2d(
            target,
            kernel_size=3,
            stride=1,
            padding=1,
        )

        gt_boundary = dilated - eroded

        intersection = (
            pred * target * gt_boundary
        ).sum(dim=(1, 2, 3))

        pred_sum = (
            pred * gt_boundary
        ).sum(dim=(1, 2, 3))

        target_sum = (
            target * gt_boundary
        ).sum(dim=(1, 2, 3))

        union = pred_sum + target_sum - intersection

        boundary_iou = (
            intersection + 1e-6
        ) / (
            union + 1e-6
        )

        return (1.0 - boundary_iou).mean()


def get_lr(optimizer):
    for param_group in optimizer.param_groups:
        return param_group["lr"]


def train_one_epoch(
    model,
    loader,
    criterion_main,
    criterion_bound,
    optimizer,
    scaler,
    device,
    epoch,
):
    model.train()
    running_loss = 0.0

    loop = tqdm(
        loader,
        desc=f"Epoch {epoch + 1}/{Config.EPOCHS} [Train]",
        leave=True,
    )

    for images, masks in loop:
        images = images.to(device)
        masks = masks.to(device)

        if masks.ndim == 3:
            masks = masks.unsqueeze(1)
        elif masks.ndim == 4 and masks.shape[-1] == 1:
            masks = masks.permute(0, 3, 1, 2)

        optimizer.zero_grad(set_to_none=True)

        with torch.autocast(
            device_type="cuda",
            dtype=torch.float16,
            enabled=device.type == "cuda",
        ):
            preds = model(images)

            if isinstance(preds, (tuple, list)):
                out1, out2, out3, out4 = preds

                loss1 = (
                    criterion_main(out1, masks)
                    + 0.3 * criterion_bound(out1, masks)
                )

                loss2 = criterion_main(out2, masks)
                loss3 = criterion_main(out3, masks)
                loss4 = criterion_main(out4, masks)

                loss = (
                    loss1
                    + 0.3 * loss2
                    + 0.2 * loss3
                    + 0.1 * loss4
                )

            else:
                loss_main = criterion_main(preds, masks)
                loss_bound = criterion_bound(preds, masks)

                loss = loss_main + 0.5 * loss_bound

        scaler.scale(loss).backward()
        scaler.step(optimizer)
        scaler.update()

        running_loss += loss.item()

        loop.set_postfix(
            loss=f"{loss.item():.4f}",
            lr=f"{get_lr(optimizer):.6f}",
        )

    return running_loss / len(loader)


def validate(model, loader, criterion, device):
    model.eval()

    running_loss = 0.0

    metrics_score = {
        "dice": 0.0,
        "iou": 0.0,
        "recall": 0.0,
        "precision": 0.0,
    }

    total_samples = 0

    with torch.no_grad():
        for images, masks in tqdm(
            loader,
            desc="[Val]",
            leave=False,
        ):
            images = images.to(device)
            masks = masks.to(device)

            if masks.ndim == 3:
                masks = masks.unsqueeze(1)
            elif masks.ndim == 4 and masks.shape[-1] == 1:
                masks = masks.permute(0, 3, 1, 2)

            batch_size = images.size(0)

            with torch.autocast(
                device_type="cuda",
                dtype=torch.float16,
                enabled=device.type == "cuda",
            ):
                preds = model(images)

                if isinstance(preds, (tuple, list)):
                    preds = preds[0]

                loss = criterion(
                    preds,
                    masks,
                )

            running_loss += loss.item() * batch_size

            batch_metrics = calculate_metrics(
                preds.float(),
                masks.float(),
                is_logits=True,
            )

            for key in metrics_score:
                metrics_score[key] += (
                    batch_metrics[key] * batch_size
                )

            total_samples += batch_size

    epoch_loss = running_loss / total_samples

    for key in metrics_score:
        metrics_score[key] /= total_samples

    return epoch_loss, metrics_score


if __name__ == "__main__":
    os.makedirs("checkpoints", exist_ok=True)
    os.makedirs("logs", exist_ok=True)

    seed_everything(Config.SEED)

    device = Config.DEVICE

    if isinstance(device, str):
        device = torch.device(device)

    print("=" * 70)
    print("Controlled D4 Placement Experiment")
    print("=" * 70)
    print(f"Experiment Name      : {Config.EXP_NAME}")
    print(f"Seed                 : {Config.SEED}")
    print(f"IMG_SIZE             : {Config.IMG_SIZE}")
    print(f"D4 Mamba Position    : {Config.D4_MAMBA_POS}")
    print(f"Layer3 Mamba         : False")
    print(f"Bottleneck Mamba     : {Config.USE_BOTTLENECK_MAMBA}")
    print(f"Conv Refine          : {Config.USE_CONV_REFINE}")
    print("=" * 70)

    train_loader, val_loader = get_loaders(Config)

    print(
        f"Data loading complete: "
        f"Train {len(train_loader)} batches | "
        f"Val {len(val_loader)} batches"
    )

    model = UNet(
        n_classes=Config.NUM_CLASSES,
        d4_mamba_pos=Config.D4_MAMBA_POS,
        use_layer3_mamba=False,
        use_bottleneck_mamba=Config.USE_BOTTLENECK_MAMBA,
        use_conv_refine=Config.USE_CONV_REFINE,
        pretrained=True,
    ).to(device)

    trainable_params = sum(
        p.numel()
        for p in model.parameters()
        if p.requires_grad
    )

    print(
        f"Trainable Params      : "
        f"{trainable_params / 1e6:.2f} M"
    )
    print("=" * 70)

    optimizer = optim.AdamW(
        model.parameters(),
        lr=Config.LEARNING_RATE,
        weight_decay=1e-4,
    )

    scheduler = optim.lr_scheduler.CosineAnnealingLR(
        optimizer,
        T_max=Config.EPOCHS,
        eta_min=1e-6,
    )

    criterion_main = StructureLoss()
    criterion_bound = BoundaryIoULoss().to(device)

    scaler = GradScaler()

    best_dice = 0.0
    best_epoch = 0

    best_ckpt_path = (
        f"checkpoints/"
        f"best_model_{Config.EXP_NAME}.pth"
    )

    last_ckpt_path = (
        f"checkpoints/"
        f"last_model_{Config.EXP_NAME}.pth"
    )

    for epoch in range(Config.EPOCHS):
        train_loss = train_one_epoch(
            model=model,
            loader=train_loader,
            criterion_main=criterion_main,
            criterion_bound=criterion_bound,
            optimizer=optimizer,
            scaler=scaler,
            device=device,
            epoch=epoch,
        )

        val_loss, metrics = validate(
            model=model,
            loader=val_loader,
            criterion=criterion_main,
            device=device,
        )

        scheduler.step()
        current_lr = get_lr(optimizer)

        save_msg = ""

        if metrics["dice"] > best_dice:
            best_dice = metrics["dice"]
            best_epoch = epoch + 1

            torch.save(
                model.state_dict(),
                best_ckpt_path,
            )

            save_msg = "Best Dice!"

        torch.save(
            model.state_dict(),
            last_ckpt_path,
        )

        print(f"\nEpoch {epoch + 1} Summary:")
        print(
            f"   Train Loss: {train_loss:.4f} | "
            f"Val Loss: {val_loss:.4f}"
        )
        print(
            f"   Dice: {metrics['dice']:.4f} | "
            f"IoU: {metrics['iou']:.4f}"
        )
        print(
            f"   Recall: {metrics['recall']:.4f} | "
            f"Precision: {metrics['precision']:.4f}"
        )
        print(
            f"   LR: {current_lr:.6f} | "
            f"{save_msg}"
        )
        print("-" * 70)

    print("\n" + "=" * 70)
    print("Training Finished")
    print("=" * 70)
    print(f"Experiment      : {Config.EXP_NAME}")
    print(f"D4 Position     : {Config.D4_MAMBA_POS}")
    print(f"Best Val Dice   : {best_dice:.4f}")
    print(f"Best Epoch      : {best_epoch}")
    print(f"Best Checkpoint : {best_ckpt_path}")
    print(f"Last Checkpoint : {last_ckpt_path}")
    print("=" * 70)