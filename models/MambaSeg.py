...
class MambaSeg_UNet(nn.Module):
    def __init__(
        ...
        print("=" * 50)
        print("Building Model Variants (Full Ablation Base)...")
        print("[*] ResNet34 Encoder   : True")
    ...
    def forward(self, x):
        ...
        if self.training:
            out2 = F.interpolate(self.head2(d2), size=x.shape[2:], mode="bilinear", align_corners=False)
            out3 = F.interpolate(self.head3(d3), size=x.shape[2:], mode="bilinear", align_corners=False)
            out4 = F.interpolate(self.head4(d4), size=x.shape[2:], mode="bilinear", align_corners=False)
